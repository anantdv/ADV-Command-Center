...
export function MissingFieldsForm({part}:{part:MissingFieldsPart}){
  ...
  const submit=()=>{
    try{
      const parsed=Object.fromEntries(part.fields.map(field=>[field.fieldname,field.fieldtype==='Table'?JSON.parse(values[field.fieldname]):values[field.fieldname]]))
