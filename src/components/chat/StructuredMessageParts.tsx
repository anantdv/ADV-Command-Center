...
function DynamicChart({part}:{part:ChartPart}){
  ...
  return <div className="rounded-xl border border-slate-200 bg-white p-4"><div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-bold text-slate-800">{part.title||'Chart preview'}</p><p className="mt-0.5 text-[10px] text-slate-400">Generated from available result rows</p></div><span className="rounded-full bg-indigo-50 px-2 py-1 text-[9px] font-bold text-indigo-600"><Sparkles size={9} className="mr-1 inline"/>Generated by AI</span></div><div className="h-52"><ResponsiveContainer width="100%" height="100%">{part.chart_type==='line'?<LineChart {...common}>{axes}<Line type="monotone" dataKey={part.y_key} stroke={chartPalette[0]} strokeWidth={2.5} dot={{r:3}}/></LineChart>:part.chart_type==='area'?<AreaChart {...common}>{axes}<Area type="monotone" dataKey={part.y_key} stroke={chartPalette[4]} fill={`${chartPalette[4]}24`} strokeWidth={2}/></AreaChart>:part.chart_type==='pie'||part.chart_type==='donut'?<PieChart><Tooltip/><Pie data={part.data} dataKey={part.y_key} nameKey={part.x_key} innerRadius={part.chart_type==='donut'?48:0} outerRadius={78} label>{part.data.map((_,index)=><Cell key={index} fill={chartPalette[index%chartPalette.length]}/>)}</Pie></PieChart>:<BarChart {...common}>{axes}<Bar dataKey={part.y_key} radius={[6,6,0,0]}>{part.data.map((_,index)=><Cell key={index} fill={chartPalette[index%chartPalette.length]}/>)}</Bar></BarChart>}</ResponsiveContainer></div></div>
}
