...
export function PermissionBadge({ label = 'Permission Checked', allowed = true, reason }: { label?: string; allowed?: boolean; reason?: string }) { if(allowed&&label==='Permission Checked')return null;return <span title={reason} className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold ${allowed?'border-indigo-100 bg-indigo-50 text-indigo-700':'border-amber-100 bg-amber-50 text-amber-700'}`}>{allowed?<CheckCircle2 size={11}/>:<LockKeyhole size={11}/>} {allowed?label:'Permission Restricted'}</span> }
