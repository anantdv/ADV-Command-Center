...
export function ModuleDoctypeCard({ moduleName, item }: { moduleName: string; item: ModuleDoctypeInfo }) {
  ...
  return <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-200 hover:shadow-md">
    ...
    <div className="mt-4 flex gap-2">
      ...
      {item.canCreate&&<button onClick={create} className="flex size-9 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 hover:bg-indigo-100" title="Create draft in Command Center"><Plus size={15}/></button>}
    </div>
  </article>
}
