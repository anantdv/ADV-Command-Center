...
export const dashboardKeys={all:['dashboard'] as const,overview:['dashboard','overview'] as const,widgets:['dashboard','widgets'] as const}
const invalidate=(client:ReturnType<typeof useQueryClient>)=>Promise.all([client.invalidateQueries({queryKey:dashboardKeys.overview}),client.invalidateQueries({queryKey:dashboardKeys.widgets})])
export const useDashboardOverview=()=>useQuery({queryKey:dashboardKeys.overview,queryFn:getDashboardOverview})
