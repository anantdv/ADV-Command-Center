...
export const moduleKeys={all:['modules'] as const,detail:(name:string)=>['modules',name] as const,dashboard:(name:string,from?:string,to?:string)=>['modules',name,'dashboard',from||'',to||''] as const,doctypes:(name:string)=>['modules',name,'doctypes'] as const,doctypeRecords:(moduleName:string,doctype:string,page:number,search?:string)=>['modules',moduleName,'doctype',doctype,page,search||''] as const,records:(name:string)=>['modules',name,'records'] as const,reports:(name:string)=>['modules',name,'reports'] as const}
...
export const useModuleDashboard=(name:string)=>{const range=useAppStore(state=>state.dateRange);return useQuery({queryKey:moduleKeys.dashboard(name,range.from,range.to),queryFn:()=>getModuleDashboard(name,range),enabled:Boolean(name)})}
