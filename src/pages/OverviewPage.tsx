...
export function OverviewPage(){
 ...
 return <><PageHeader eyebrow="Live ERPNext overview" title="Good Morning, Admin" description="Permission-aware KPIs and widgets refreshed from your ERPNext data." actions={<div className="flex gap-2"><button onClick={()=>void overview.refetch()} className="btn-secondary"><RefreshCw size={15}/>Refresh all</button><button onClick={()=>navigate('/command-center')} className="btn-primary"><Sparkles size={16}/>Ask Tinni</button></div>}/>
 ...
 <div className="mt-7 mb-4 flex items-center justify-between"><div><h2 className="font-[Manrope] text-lg font-bold">Dashboard widgets</h2><p className="mt-1 text-xs text-slate-400">Pinned results and permission-aware defaults</p></div></div>
